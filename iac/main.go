...
func main() {
	...
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		repository, err := createRepository(ctx, "pulumi-gh")
		if err != nil {
			return err
		}

		_, err = github.NewActionsSecret(ctx, "actionSecret", &github.ActionsSecretArgs{
			Repository: repository.Name,
			SecretName: pulumi.String("PULUMI_CONFIG_PASSPHRASE"),
		})
		if err != nil {
			return err
		}
		...
		issueLabel2, err := github.NewIssueLabel(ctx, "issueLabel2", &github.IssueLabelArgs{
		...
		ctx.Export("issueLabel2", issueLabel2.Name)
